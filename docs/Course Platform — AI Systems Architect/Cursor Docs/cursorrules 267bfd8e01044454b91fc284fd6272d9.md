...
Copy this entire page content into a file named `.cursorrules` in your project root.
...
- Install packages with `npm install [package]` — no pinned versions
- Check `npm outdated` before starting new chunks
- Do not add analytics packages (custom Event table)
- Do not add community/forum packages (using Discord)

## Documentation Reference

- Architecture decisions: see [ARCHITECTURE.md](http://ARCHITECTURE.md)
- Database schema: see DATABASE_[SCHEMA.md](http://SCHEMA.md)
- API integrations: see API_[CONTRACTS.md](http://CONTRACTS.md)
- Task navigation: see TASK_[ROUTER.md](http://ROUTER.md)